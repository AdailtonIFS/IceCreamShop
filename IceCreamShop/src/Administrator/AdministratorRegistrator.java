...
public class AdministratorRegistrator extends JFrame {
	 ...
	 */
	public AdministratorRegistrator() throws ParseException {

		setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
		setTitle("CADASTRO DE CLIENTES");

		//MASCARAS
		ftmTelefone = new MaskFormatter("(##)#####-####");//MASCARA DO NÚMERO DE CELULAR
		...
		NomeCl = new JTextField();//CAMPO PRO USUÁRIO DIGITAR O NOME
		...
		JLabel labelCod = new JLabel("C\u00D3DIGO:");//LABEL DO CÓDIGO
		...
		CodCl = new JTextField();//CAMPO DO CÓDIGO
		...
		JButton CANCELAR = new JButton("CANCELAR");//CRIANDO BOTÃO DE CANCELAR
		CANCELAR.setBorder(new LineBorder(new Color(0, 0, 0), 2, true));
		CANCELAR.setFont(new Font("Arial", Font.BOLD, 13));
		CANCELAR.setBounds(51, 291, 113, 41);//TAMANHO DO BOTÃO
		panelBaixo.add(CANCELAR);//ADICIONANDO O BOTÃO AO PAINEL DE BAIXO
		CANCELAR.setBackground(UIManager.getColor("Button.background"));//COR DO BOTÃO
		

		//AÇÃO DO BOTAO DE CANCELAR
		CANCELAR.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			
				Object[] options = { "SIM", "NÃO" };//CRIANDO UM OBJETO DE OPÇÕES
				int Cancel = JOptionPane.showOptionDialog(null, "TEM CERTEZA QUE DESEJA CANCELAR?", "ALERTA", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE , null, options, options[0]);//JOptionPane DE CONFIRMAÇÃO
			
				//VERIFICANDO RESPOSTA DO USUÁRIO
		if(Cancel == JOptionPane.YES_OPTION) {//RESPOSTA SIM
			JOptionPane.showMessageDialog(null,"CADASTRO CANCELADO");//EXIBINDO CONFIRMAÇÃO
		...
		SALVAR.setBounds(255, 291, 114, 41);//TAMANHO DO BOTÃO
		panelBaixo.add(SALVAR);//ADICIONANDO O BOTÃO AO PAINEL DE BAIXO
		SALVAR.setForeground(Color.BLACK);//FUNDO DO BOTÃO
		SALVAR.setBackground(UIManager.getColor("Button.background"));//COR DO BOTÃO
